...
//    public final static float ORIGIN_LON = -79.79637222222222f;
    ...
    // public final static String BACKGROUND_IMAGE_FILENAME = "/home/owens/gascolahires.jpg";
    public final static String BACKGROUND_IMAGE_FILENAME = "F:\\laptop\\owens\\gascola_data\\gascola_mosaic.jpg";
...
// new
    ...
    public UavMapDisplay(String vcBridgeServerHostname, int port, String playbackFilename, double speedup, int maxUavTrailSize) {
	...
	try {
	    backgroundSource = ImageIO.read(new File(BACKGROUND_IMAGE_FILENAME));
	} 
	catch(java.io.IOException e) {
	    Debug.debug(3,"IOException loading background image, from '"+BACKGROUND_IMAGE_FILENAME+"',  e="+e);
	    e.printStackTrace();
	}
	...
	//      	Debug.debug(1,"mouse entered e="+e);
    }
    public void mouseExited(MouseEvent e) {
	//      	Debug.debug(1,"mouse exited e="+e);
    }
    public void mouseClicked(MouseEvent e) {
	//      	Debug.debug(1,"mouse clicked e="+e);
    }
	...
	if(null !=  curTarget) {
	    ...
	    Debug.debug(1,"Updated target to "+curTarget);
	}
	else 
	    Debug.debug(1,"Dragging, curTarget is null.");
	...
	Point2D.Double point = new Point2D.Double(localx, localy);
      	Debug.debug(1,"uavid "+uavid+" lat/lon "+latitude+","+longitude+" local "+localx+","+localy);
	...
	if((iscreenW != screenWidthCache)
	   ...
	   || (null == scaledBackground)) {
	    ...
	    Debug.debug(1, "Background image: local west "+westLocalX+" east "+eastLocalX+" north "+northLocalY+" south "+southLocalY);
	    Debug.debug(1, "Background image: screen west "+westScreenX+" east "+eastScreenX+" north "+northScreenY+" south "+southScreenY + " width "+widthScreen+" height "+heightScreen);
	...
	synchronized(uavLocs) {
		...
		//		Debug.debug(1,"paint: uavid "+uavid+" at "+x+","+y);
	    }
	}
	...
	//      	Debug.debug("paint: total="+totalTime,1,this);
    }    
	...
	if(words[0].equalsIgnoreCase("ERROR")) {
	    Debug.debug(3," Got an error back from VCBridgeServer = "+line);
	    return;
	}
	if(words[0].equalsIgnoreCase("NAVDATA")) {
	    if(words.length < 9) {
		Debug.debug(3," Got a NAVDATA line from VCBridgeServer, but not enough fields, should be 9, only "+words.length+", original line="+line);
		return;
	    }
	...
	Debug.debug(1,"Unknown line from VCBridgeServer="+line);
    }
	...
	while(true) {
	    Debug.debug(1,"Opening connection to "+serverHostname+" port "+serverPort);
		...
		Debug.debug(1,"        Exception opening connection to "+serverHostname+" port "+serverPort+", will wait 1000ms and try again, exception is;");
		e.printStackTrace(System.err);
		try { Thread.sleep(1000); } catch(Exception e2) {}
		continue;
	    }

	    Debug.debug(1,"About to start reading/writing to/from socket.");
		...
		try {
		    ...
		    if(null == line) {
			Debug.debug(3,"Socket to VCBridgeServer has closed on us!");
			break;
		    }
		    processInputLine(line);
		}
		catch (Exception e){
		    Debug.debug(3,"Exception reading socket from VCBridgeServer, e="+e);
		    e.printStackTrace();
		    break;
		}
	    }
		...
		Debug.debug(2,"Exception closing socket.");
		e.printStackTrace(System.err);
	    }
	...
	for(int loopi=0; loopi < argv.length; loopi++) {
	    if((argv[loopi].equalsIgnoreCase("-vcbridgeserverip")) || (argv[loopi].equalsIgnoreCase("--vcbridgeserverip")) && ((loopi+1) < argv.length)) {
	...
	display = new UavMapDisplay(vcBridgeServerHostname, vcBridgeServerPort, playbackFilename, speedup,maxUavTrailSize);
	display.start();
    }
