...
public class Main {
    ...
    public void decodePacket(long packetTime, byte[] buf, int length, DecodedPacket dp) {
        ...
        try {
            dp.passThroughPacketType = NetByte.decodeInt(buf, PASS_THROUGH_PACKET_TYPE_OFFSET);
            dp.passThroughDataSize = NetByte.decodeInt(buf, PASS_THROUGH_DATA_SIZE_OFFSET);

            dp.startByte = buf[OFF_START];
            dp.type = 0x000000FF & ((int) buf[OFF_TYPE]);
            int srcAddrLow = buf[OFF_SRC_ADDR_LOW];
            int srcAddrHigh = buf[OFF_SRC_ADDR_HIGH];
            dp.srcAddr = (srcAddrHigh << 8) | srcAddrLow;
            int destAddrLow = buf[OFF_DEST_ADDR_LOW];
            int destAddrHigh = buf[OFF_DEST_ADDR_HIGH];
            dp.destAddr = (destAddrHigh << 8) | destAddrLow;

            dp.endByte = buf[length - 1 - MINUS_OFF_END];
            dp.xorCheckLowByte = 0xFF & buf[length - 1 - MINUS_OFF_XOR_LOW_BYTE];
            dp.xorCheckHighByte = 0xFF & buf[length - 1 - MINUS_OFF_XOR_HIGH_BYTE];
            dp.xorCheck = (dp.xorCheckHighByte << 8) | dp.xorCheckLowByte;
            dp.packetID = buf[length - 1 - MINUS_OFF_PACKET_ID];
...
// 	    dp.gpsVelocity = (dp.gpsVelocity/20) - 10;
// 	    dp.gpsAlt = (dp.gpsAlt/6) - 1000;
// 	    dp.gpsHeading = Math.toDegrees(((double)dp.gpsHeading)/1000);
// 	    dp.temperatureRGyro = ((dp.temperatureRGyro/2.8) - 10);	// to Celsius
// 	    dp.temperatureRGyro = ((9.0/5.0)* (dp.temperatureRGyro)) + 32;	// to farenheit
// 	    dp.homeAlt = (dp.homeAlt/6) - 1000;


            dp.gpsVelocity = NetByte.decodeShort(buf, OFF_GPS_VELOCITY);
            dp.gpsVelocity = (dp.gpsVelocity / 20) - 10;
            dp.gpsAlt = NetByte.decodeShort(buf, OFF_GPS_ALT);
            dp.gpsAlt = (dp.gpsAlt / 6) - 1000;
            dp.gpsHeading = NetByte.decodeShort(buf, OFF_GPS_HEADING);
            dp.gpsHeading = Math.toDegrees(((double) dp.gpsHeading) / 1000);
            dp.temperatureRGyro = (float) (0xFF & buf[OFF_TEMPERATURE_R_GYRO]);
            dp.temperatureRGyro = ((dp.temperatureRGyro / 2.8) - 10);	 // to Celsius
            dp.temperatureRGyro = ((9.0 / 5.0) * (dp.temperatureRGyro)) + 32;	 // to farenheit
            dp.gpsLat = NetByte.decodeFloat(buf, OFF_GPS_LAT);
            dp.gpsLon = NetByte.decodeFloat(buf, OFF_GPS_LON);
            dp.gpsLatHome = NetByte.decodeFloat(buf, OFF_GPS_LAT_HOME);
            dp.gpsLonHome = NetByte.decodeFloat(buf, OFF_GPS_LON_HOME);
            dp.homeAlt = NetByte.decodeShort(buf, OFF_HOME_ALT);
            dp.homeAlt = (dp.homeAlt / 6) - 1000;
...
// 	infoStream.println();
    ...
    public Main(String logFileName) {
...
//     }
    ...
    private static void printUsageAndExit(String errorMsg) {
        ...
        System.err.println("Usage: java Main -l logfilename");
    ...
    public static long readLittleLong(byte b0, byte b1, byte b2, byte b3, byte b4, byte b5, byte b6, byte b7) {
        return (0xFF & b0) | ((0xFF & b1) << 8) | ((0xFF & b2) << 16) | ((0xFF & b3) << 24) | ((0xFF & b4) << 32) | ((0xFF & b5) << 40) | ((0xFF & b6) << 48) | ((0xFF & b7) << 56);
    }

    public static float readLittleFloat(byte b0, byte b1, byte b2, byte b3) {
        return Float.intBitsToFloat((0xFF & b0) | ((0xFF & b1) << 8) | ((0xFF & b2) << 16) | ((0xFF & b3) << 24));
    }

    public static double readLittleDouble(byte b0, byte b1, byte b2, byte b3, byte b4, byte b5, byte b6, byte b7) {
        return Double.longBitsToDouble((0xFF & b0) | ((0xFF & b1) << 8) | ((0xFF & b2) << 16) | ((0xFF & b3) << 24) | ((0xFF & b4) << 32) | ((0xFF & b5) << 40) | ((0xFF & b6) << 48) | ((0xFF & b7) << 56));
    }
    ...
    public static short readBigShort(byte b0, byte b1) {
        return (short) ((0xFF & b1) | ((0xFF & b0) << 8));
    }
    ...
    public static long readBigLong(byte b0, byte b1, byte b2, byte b3, byte b4, byte b5, byte b6, byte b7) {
        return (0xFF & b7) | ((0xFF & b6) << 8) | ((0xFF & b5) << 16) | ((0xFF & b4) << 24) | ((0xFF & b3) << 32) | ((0xFF & b2) << 40) | ((0xFF & b1) << 48) | ((0xFF & b0) << 56);
    }

    public static float readBigFloat(byte b0, byte b1, byte b2, byte b3) {
        return Float.intBitsToFloat((0xFF & b3) | ((0xFF & b2) << 8) | ((0xFF & b1) << 16) | ((0xFF & b0) << 24));
    }

    public static double readBigDouble(byte b0, byte b1, byte b2, byte b3, byte b4, byte b5, byte b6, byte b7) {
        return Double.longBitsToDouble((0xFF & b7) | ((0xFF & b6) << 8) | ((0xFF & b5) << 16) | ((0xFF & b4) << 24) | ((0xFF & b3) << 32) | ((0xFF & b2) << 40) | ((0xFF & b1) << 48) | ((0xFF & b0) << 56));
    }
//     public static int readBigUnsignedByteToInt(byte b0) {
// 	return 0xFF & b0;
//     }
