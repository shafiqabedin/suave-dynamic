...
public class DEMxyz {
    ...
    public DEMxyz(String inputFilename) {
    ...
    public void go(String outputFilename) {
        ...
        try {
            ...
            Logger.getLogger(DEMxyz.class.getName()).log(Level.SEVERE, "FileNotFound, filename=" + inputFilename, ex);
        }
        ...
        try {
            ...
            Logger.getLogger(DEMxyz.class.getName()).log(Level.SEVERE, "Couldn't open outputfile! filename=" + outputFilename, ex);
        }
        ...
        try {
            ...
            parseDoubleListFast(3, values, line);
...
//            }
            ...
            Logger.getLogger(DEMxyz.class.getName()).log(Level.SEVERE, null, ex);
        }
        writer.flush();
        writer.close();
    }

    public void readVBS2xyz(int width, int height) {
        ...
        try {
            for (int loopx = 0; loopx < width; loopx++) {
                Debug.debug(1, "reading line " + loopx);
                ...
                for (int loopy = 0; loopy < height; loopy++) {
                    ...
                    verticeData[(int) x][(int) y][0] = x;
                    verticeData[(int) x][(int) y][1] = y;
                    verticeData[(int) x][(int) y][2] = z;
                }
            }
        } catch (IOException ex) {
            Logger.getLogger(DEMxyz.class.getName()).log(Level.SEVERE, null, ex);
        ...
        try {
            while (true) {
                ...
                if (line.startsWith("width=")) {
                    ...
                    parseDoubleListFast(5, xyzll, line.substring(7));
            ...
            for (int loopx = 0; loopx < width; loopx++) {
                ...
                if (percentLoaded >= messageAt) {
                    Debug.debug(1, "loadXyz: percent loaded = " + percentLoaded);
    ...
    public void writeXYZ(String outputFilename) {
    ...
    public void writeZZZ(String outputFilename) {
        FileWriter fileWriter = null;
        PrintWriter writer = null;
        try {
            fileWriter = new FileWriter(outputFilename);
        ...
        int width = verticeData.length;
        int height = verticeData[0].length;
        float stepy = verticeData[0][1][1] - verticeData[0][0][1];
        writer.println("width=" + width);
        writer.println("height=" + height);
        writer.println("stepy=" + stepy);
        writer.println("DATA");
        double lastx;
        double lasty;
        double firstElev;
        ...
        for (int loopx = 0; loopx < width; loopx++) {
            Debug.debug(1, "writing" + loopx);
            lastx = verticeData[loopx][0][0];
            lasty = verticeData[loopx][0][1];
            firstElev = verticeData[loopx][0][2];
            writer.print(elevFmt.format(lastx) + " " + elevFmt.format(lasty) + " " + elevFmt.format(firstElev));
            for (int loopy = 1; loopy < height; loopy++) {
//                System.err.print(" "+verticeData[loopx][loopy][2]);
                writer.print(" " + elevFmt.format(firstElev - verticeData[loopx][loopy][2]));
            }
//            System.err.println();
            writer.println();
        }
        ...
        try {
            ...
            while (true) {
                ...
                if (line.startsWith("width=")) {
                    ...
                    height = Integer.parseInt(line.substring(7));
                    Debug.debug(1, "height=" + height);
                } else if (line.startsWith("stepy=")) {
                    stepy = Float.parseFloat(line.substring(6));
                    Debug.debug(1, "stepy=" + stepy);
                }
            }
            verticeData = new float[width][height][3];
            ...
            for (int loopx = 0; loopx < width; loopx++) {
                line = bufferedReader.readLine();
                parseDoubleListFast(height + 2, values, line);

                float x = (float) values[0];
                float y = (float) values[1];
                float firstElev = (float) values[2];
                verticeData[loopx][0][0] = x;
                verticeData[loopx][0][1] = y;
                verticeData[loopx][0][2] = firstElev;
//                System.err.print("Parsed line " + loopx + " " + x + " " + y + " " + firstElev);
                for (int loopy = 1; loopy < height; loopy++) {
                    float newY = y + (stepy * loopy);
//                    System.err.print("parsing " + loopx + " " + newY);
                    verticeData[loopx][loopy][0] = x;
                    verticeData[loopx][loopy][1] = newY;
                    verticeData[loopx][loopy][2] = firstElev - (float) values[loopy + 2];
//                    System.err.print(" (" + verticeData[loopx][loopy][0] + ", " + verticeData[loopx][loopy][1] + ", " + verticeData[loopx][loopy][2] + ")");
//                    System.err.print(" "+values[loopy+2]);
                }
//                System.err.println();
            }
        } catch (IOException ex) {
...
//        v = new DEMxyz("g:\\suave\\trunk\\warminster\\heightmap_2010_09_08_013723.xyz");
