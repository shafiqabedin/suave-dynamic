...
public class GLDisplay {
    ...
    private static final int FRAMES_PER_SECOND_SPEED = 200;
    ...
    public static GLDisplay createGLDisplay(String title, boolean videoMode, JPanel subPanel) {
        ...
        int displayWidth = screenSize.width - 10;
        int displayHeight = screenSize.height - 30;
        if (videoMode) {
            displayWidth = 720;
            displayHeight = 480;
        }
        return new GLDisplay(title, displayWidth, displayHeight, fullscreen, subPanel);
    }

    private GLDisplay(String title, int width, int height, boolean fullscreen, JPanel subPanel) {
        glCanvas = new GLCanvas(new GLCapabilities());
        if (subPanel != null) {
            int subPanelHeight = subPanel.getPreferredSize().height;
            glCanvas.setSize(width, height - subPanelHeight);
        } else {
        ...
        animator = new FPSAnimator(glCanvas, FRAMES_PER_SECOND_SPEED);
